##########################################################################
# Master Data Science: Data Wrangling
##########################################################################


# Cargando librerias -------------------------------------------------

list.of.packages <- c("R.utils", "readr", "data.table", "dplyr", "stringr", "foreach", "doParallel", "tidyverse", "doParallel", "foreach", "sqldf")
...
( data_path <- file.path('downloads') )  # Creamos la ruta donde estan los archivos a cargar, en python utilizamos la libreria os
( files <- list.files(data_path, pattern = '*.csv', full.names = T) )

system.time( flights <- lapply(files, fread) ) 
system.time( flights <- lapply(files, fread, nThread=4) ) # utilizando 4 nucleos
...
jfk <- read_csv_chunked("downloads/2008.csv",
                        chunk_size = 50000,
...
filter(flights, UniqueCarrier %in% c('AA', 'UA')) # puedes correr una funcion que arroje un vector
...
View(head(filter(flights, TaxiIn + TaxiOut > AirTime)))
...
Date <- function(a,b,c)
{
  if (as.numeric(b) < 10)
  {
    if (as.numeric(c) < 10)
    {
      print( paste( a, "-0", b, "-0", c, sep = "") )
    }
    else
    {
      print( paste( a, "-0", b, "-", c, sep = "") )
    }
  }
  else 
  {
...
foo <- mutate(jfk,
              Date = as.Date(Date(Year,Month,DayofMonth)))
...
hist(na_array_delay$ArrDelay) # opcional break = 20
...
summarise(taxi, maxTaxiing = max(abs(TaxiOut - TaxiIn)))
...
summarise(aa, Total = n(), Cancel = sum(Cancelled), PctCancel = sum(Cancelled) / n() * 100, PromArr = mean(ArrDelay, na.rm = T))
...
summarise(aa, TotCancel = sum(CancellationCode == 'D',na.rm = T))
...
flights %>%
  ...
  group_by(UniqueCarrier) %>%
  summarise(Cant = n(), AvgDelay = mean(ArrDelay)) %>%
  mutate(RankAvg = rank(AvgDelay)) %>%
  arrange(RankAvg)
...
foo <- flights %>% 
  ...
  mutate_at(vars(ends_with("Delay")), funs(./2)) # Hay variantes de mutate_at, _it, etc.
foo
...
outliers <- boxplot.stats(flights$ActualElapsedTime)$out # criterio outliers media +- 3 * desviacion estandar(esto se puede modificar)
length(outliers)
outliers
...
flights2 %>%
  group_by(UniqueCarrier) %>%
  top_n(1, DepDelay) %>%
  left_join(airports %>% select(1, airport, city, state), by = c("Origin" = "iata"))
